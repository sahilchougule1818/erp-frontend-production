        ...
        <div className="px-6 py-4 space-y-4" style={{ flex: 1, overflowY: 'auto' }}>
          <div className="space-y-3 bg-red-50 border border-red-200 rounded-lg p-4">
            ...
            <div className="space-y-2">
              <p className="text-xs bg-gray-100 text-gray-600 px-2 py-0.5 rounded font-medium inline-block">
                {selectedSale.booking_id}
              </p>
              <span className="text-sm text-gray-600 ml-2">{selectedSale.customer_name}</span>
            </div>
    ...
    <ModalLayout title="Manage Instant Sale" width="w-[600px]">
      <div className="px-6 py-4 space-y-4" style={{ flex: 1, overflowY: 'auto' }}>
        ...
        <div className="flex items-center gap-2 -mt-2">
          ...
          <span className="text-sm text-gray-500">{selectedSale.customer_name}</span>
        </div>
      {/* Financial Summary */}
      <div className="space-y-2">
        ...
        <div className="border rounded-md p-4 shadow-sm bg-white">
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <p className="text-sm font-semibold text-gray-900">{selectedSale.plant_name || '—'}</p>
            </div>
        ...
        <div className="border rounded-md p-3 space-y-2">
          <Button type="button" variant="outline" onClick={() => setIsPaymentHistoryExpanded(!isPaymentHistoryExpanded)} className="w-full justify-between h-8 text-sm">
            <div className="flex items-center gap-2">
              ...
              {payments.length > 0 && <span className="text-xs text-slate-500">· ₹{Number(selectedSale.paid_amount).toLocaleString()} paid</span>}
            </div>
                ...
                <div key={payment.transaction_number} className="flex items-center justify-between p-2 hover:bg-gray-50 rounded">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="secondary" className="text-xs font-medium">{payment.payment_method}</Badge>
                      {idx === 0 && <Badge variant="outline" className="text-xs text-emerald-700 border-emerald-200">Latest</Badge>}
                    </div>
      ...
      {isCancelled ? (
        <div className="bg-red-50 border border-red-200 rounded-lg p-4 text-center space-y-1">
          ...
          <p className="font-semibold text-sm text-red-700">Sale Cancelled</p>
          {selectedSale.cancellation_reason && (
            <p className="text-xs text-red-500">Reason: {selectedSale.cancellation_reason}</p>
          )}
        </div>
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-5 text-center space-y-2">
          ...
          <p className="text-xs text-green-600">This sale has been fully paid.</p>
        </div>
      ) : (
        <div className="space-y-3">
          <h3 className="font-semibold text-sm text-gray-700">Record New Payment</h3>
              ...
              <div className="flex rounded-lg overflow-hidden border border-gray-200">
                {[{ val: 'ADVANCE', label: 'Advance Payment', color: 'bg-green-600' }, { val: 'REGULAR', label: 'Regular Payment', color: 'bg-green-600' }].map(({ val, label, color }, i) => (
                  <button
                    ...
                    className={`flex-1 px-4 py-2 text-sm font-medium transition-colors ${i > 0 ? 'border-l border-gray-200' : ''} ${formData.payment_type === val ? `${color} text-white` : 'bg-white text-gray-600 hover:bg-gray-50'}`}
                  >
                    {label}
                  </button>
                ))}
