...
export const indoorApi = {
  unified: {
    getRegistry:          (params?: { page?: number }) => apiClient.get('/indoor/unified/registry', { params }),
    getDashboardStats:    (params?: string) => apiClient.get(`/indoor/unified/dashboard/stats${params || ''}`),
    getStageDistribution: () => apiClient.get('/indoor/unified/dashboard/stage-distribution'),
    getReadyForExport:    () => apiClient.get('/indoor/unified/dashboard/ready-for-export'),
    createBatch:          (data: any) => apiClient.post('/indoor/unified/batch/create', data),
    subculture:           (data: any) => apiClient.post('/indoor/unified/batch/subculture', data),
    incubate:             (data: any) => apiClient.post('/indoor/unified/batch/incubate', data),
    exportBatch:          (data: any) => apiClient.post('/indoor/unified/batch/export', data),
    unexportBatch:        (data: any) => apiClient.post('/indoor/unified/batch/unexport', data),
    undoLastAction:       (data: any) => apiClient.post('/indoor/unified/undo', data),
    getUndoPreview:       (batchCode: string) => apiClient.get(`/indoor/unified/undo-preview/${batchCode}`),
    deleteBatch:          (batchCode: string) => apiClient.delete(`/indoor/unified/batch/${batchCode}`)
  },

  phaseViews: {
    getSubculturing: (page?: number, limit?: number) => apiClient.get('/indoor/views/subculturing', { params: { page, limit } }),
    getIncubation:   (page?: number, limit?: number) => apiClient.get('/indoor/views/incubation', { params: { page, limit } }),
